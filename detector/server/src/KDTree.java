...
public class KDTree<T> {
	...
	private class NodeKD {
		...
		private float[] maxBounds, minBounds;
		private Object[] bucketValues;
		private float[][] bucketKeys;
		private boolean isLeaf;
		private int current, sliceDimension;
		private float slice;
 
		private NodeKD(int parentSliceDimension) {
			bucketValues = new Object[bucketSize];
			bucketKeys = new float[bucketSize][];

			sliceDimension = (parentSliceDimension + 1) % dimensions; // Added by Reed
		...
		private void addPoint(float[] key, Object val) {
			if(isLeaf) {
		...
		private void addLeafPoint(float[] key, Object val) {
			extendBounds(key);
		...
		private Object[] range(float[] upper, float[] lower) {
		...
		public boolean contains(float[] upper, float[] lower, float[] point) {
		...
		public boolean intersects(float[] up0, float[] low0, float[] up1, float[] low1) {
			if (up1 == null || low1 == null) return false; //added by Reed
		...
		private void splitLeaf() {
			left = new NodeKD(sliceDimension);
			right = new NodeKD(sliceDimension);
 
			slice = (maxBounds[sliceDimension] + minBounds[sliceDimension]) * 0.5f;
		...
		private void extendBounds(float[] key) {
			if (maxBounds == null) {
